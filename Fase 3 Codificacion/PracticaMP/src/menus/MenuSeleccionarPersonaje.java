...
public class MenuSeleccionarPersonaje {
    ...
    public String mostrarPagina(int n) {
        ...
        for (int i=0;i<=tamPag-1;i++){
            if (i+n*tamPag >= this.personajes.size()){ //si el elemento que se mostraría no existe en el array, no hacemos nada
                break;
            } else{ //la posición en la que vamos a buscar al personaje existe en el array
